        ...
        children: [
          ...
          GestureDetector(
            onTap: () {
              ...
              gameController.fetchAndShowRules(gameTypeId, game.name);
            },
            child: Container(
              width: 32,
              height: 32,
          ...
          Row(
            children: [
              ...
              Expanded(
                child: ElevatedButton(
                  onPressed: () async {
                    debugPrint('=== Buy Ticket Button Pressed ===');
                    String selectedNumbers =
                        gameController.selectedNumbers.value;
                    int requiredLen = game.category == '2D'
                        ? 2
                    ...
                    debugPrint('selectedNumbers: "$selectedNumbers" (length: ${selectedNumbers.length}, required: $requiredLen)');

                    if (selectedNumbers.length < requiredLen) {
                      debugPrint('Validation failed: selectedNumbers length is less than requiredLen.');
                      showToast(
                        'Please select all required numbers.'.tr,
                        title: 'Error',
                      );
                    ...
                    double amount =
                        double.tryParse(gameController.enteredAmount.value) ??
                        0.0;
                    debugPrint('enteredAmount raw: "${gameController.enteredAmount.value}", parsed: $amount (min: ${game.minBet}, max: ${game.maxBet})');
                    ...
                    if (isBetValid) {
                      ...
                      final bool validated = await cartController.validateTicket(
                        drawId: drawId,
                        gameTypeId: gameTypeId,
                        numberPrimary: numberPrimary,
                        numberSecondary: numberSecondary,
